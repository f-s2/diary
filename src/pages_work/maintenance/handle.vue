  ...
  <div class="page-body">
    ...
    <uv-form labelPosition="left" :model="formData" :rules="rules" ref="formRef" labelWidth="auto">
      ...
      <div class="form-card input-right">
        ...
        <uv-form-item @click="multiple=false; disabledIds=[]; modalState.userShow=true" label="保养人员"
                      prop="maintainUser" borderBottom>
          <span v-if="formData.maintainUserName"> {{ formData.maintainUserName }}</span>
          <span v-else style="color:#c0c4cc">请选择</span>
        </uv-form-item>
        <uv-form-item class="input-right"
                      @click="multiple=true;disabledIds=[formData.maintainUser] ;modalState.userShow = true"
                      label="协助人员"
                      borderBottom>
        ...
        <uv-form-item label="保养工时" required prop="maintainHour" borderBottom>
          <uv-input type="number" inputAlign="right" v-model="formData.maintainHour" placeholder="保养工时"
                    border="none">
          </uv-input>
        </uv-form-item>
        <uv-form-item label="保养记录" @click="handleFill" required>
...
const showTime = () => {
  datetime.value = dayjs(formData.value?.maintainTime ? formData.value.maintainTime : undefined).valueOf()
...
const handleUserOk = (data) => {
  if (multiple.value) {
    ...
    formData.value.maintainUserName = data[0]?.username
    formData.value.maintainUser = data[0]?.id
  }
...
const getInfo = () => {
  MaintenanceApi.detail(formData.value.id).then(res => {
    ...
    success: ({confirm}) => {
      confirm && MaintenanceApi.handle({
        ...
        if (res.code === 0) {
          ...
          uni.reLaunch({
            url: '/pages_work/index/index?type=0'
          })
        } else {
          uni.showToast({
            title: res.message,
