    ...
    <div class="page-body">
        <uv-form class="form input-right" ref="formRef" :rules="rules" :model="formData" label-position="left"
            ...
            <uv-form-item label="开始时间" prop="startTime" :required="true">
                ...
                <uv-datetime-picker ref="datePicker" :value="formData.startTime" mode="date" @confirm="confirm" />
...
const confirm = ({ value }) => {
    formData.value.startTime = dayjs(value).format('YYYY-MM-DD')
...
const save = () => {
    formRef.value.validate().then(() => {
        const { inspectionId, startTime } = formData.value

        InspectionkApi.updateItem({ startTime, inspectionId }).then(res => {
            if (res.code === 0) {
                uni.showToast({
                    title: '请求成功'
                })
