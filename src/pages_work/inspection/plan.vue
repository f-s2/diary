<template>
    <div class="page-body">
        <uv-form class="form input-right" ref="formRef" :rules="rules" :model="formData" label-position="left"
            labelWidth="auto">
            <uv-form-item label="点检项" prop="itemIds" :required="true">
...
const save = () => {
    ...
    formRef.value.validate().then(() => {
        const { itemList, inspectionId, id } = formData.value
        InspectionkApi.updateItem({ id, inspectionId, itemIds: itemList.map(item => item.id) }).then(res => {
            if (res.code === 0) {
                uni.showToast({
                    title: '请求成功'
                })
