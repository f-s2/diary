        ...
        <div class=" px-4 space-y-3 flex-1 overflow-auto">
            ...
            <ModuleWrapper title="盘点区域">
                <div class=" space-y-5 text-14px">
                    <div class=" flex f-c-b gap-5" v-for="item,index in detail?.areaInfoList" :key="item.areaId" @click="jump(item.areaId)">
                        <div class=" flex"><div>{{ index +1 }}、</div>{{ item.areaName }}</div>
                        <CustomTag :status="getStatus(item.completionProgress)" :progress="item.completionProgress"></CustomTag>
                    </div>
                </div>
            </ModuleWrapper>
        </div>
        <div class="mx-auto w-[calc(100%-32px)] py-3">
            <uv-button type="primary" v-if="detail?.areaInfoList.every(v => v.completionProgress === 1)">完成</uv-button>
