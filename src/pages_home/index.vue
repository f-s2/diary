        ...
        <div class="map-box">
            ...
            <div class="map-card">
                ...
                <div class="mid-items">
                    <div class="mid-item" v-for="item in completeInfo">
                        <div class="label">{{ item.name }}</div>
                        <div class="value">{{ item.transform ? item.transform(maintenanceInfo[mtIndex]?.[item.code]) :
                    maintenanceInfo[mtIndex]?.[item.code] }}</div>
                    </div>
                </div>
                <div class="map-head">
...
const completeInfo = [
    ...
    { name: '完成率', code: 'completionRate', transform: (val) => val ? parseFloat(val, 1) : 0 + '%' },
    { name: '我执行', code: 'mine' },
    { name: '占比', code: 'mineRate', transform: (val) => val ? parseFloat(val, 1) : 0 + '%' },
]
